...
import {DayPlanCategory} from "../../../app/models/recipe.ts";
import {observer} from "mobx-react-lite";


export default observer(function DayPlanFilters() {
    const {recipeStore: {predicate, setPredicate}} = useStore();

    // Convert IngredientCategory enum into dropdown options
    const categoryOptions = Object.values(DayPlanCategory).map((category) => ({
        key: category,
        text: category,
        value: category,
    }));
    ...
    return (
        <Menu vertical fluid size="large" className={styles.filterMenu}>
            ...
            {/!* Row for "All DayPlans", "Price High", and "Price Low" *!/}
            <Menu.Item>
                <div className={styles.filterButtonsRow}>
                    <Button
                        content="All DayPlans"
                        color="teal"
                        size="small"
            ...
            <Menu.Item>
                ...
                <Dropdown
                    ...
                    onChange={(_e, data) => setPredicate("category", data.value as DayPlanCategory)}
